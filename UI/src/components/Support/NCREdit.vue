            ...
            <div class="col pt-4">
              <template v-if="isEntry">
                ...
                <div>
                  ...
                  <div>
                    <img v-if="form.file.temp" :src="form.file.path" />
                    <img v-else :src="$store.state.URL + form.file.path" />
                  </div>
                </div>
              </template>
              <template v-else>
                <label>{{ form.file.name }}</label>
                <div>
                  <img :src="$store.state.URL + form.file.path" />
                </div>
              </template>
            </div>
          </div>
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col text-right pb-4">
              <p-button type="danger" @click="editCancel" round
                >Cancel</p-button
              >
              <p-button class="ml-3" type="info" @click="submitHandle" round
                >Submit</p-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    initPage() {
      ...
      this.getMillsArray();
      if (this.$store.state.bus.ncr.entry) {
        this.form = this.$store.state.bus.ncr.data;
      } else {
      ...
      if (this.form.file.temp) {
        ...
        axios
          ...
          .then(response => {
            ...
            delete data.file.__v;
            delete data.file.temp;
            axios
              .put(that.$store.state.apiURL + "ncr/" + that.form._id, data, {
                headers: {
